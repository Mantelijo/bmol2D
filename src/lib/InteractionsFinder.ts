import { DNAResidues, Polymer, PolymerKind, Residue, ResidueImplementation, ResidueMeta } from "./types/atoms";
    ...
    watsonCrickPairs(chain1: Polymer, chain2: Polymer) {
        chain1.residues.forEach(r1 => {
            ...
            if (
                ...
                (smallestDistance <= WATSON_CRICK_PAIR_CALCULATION_THRESHOLD + Math.random() && isWatsonCrickPair(bestR2, r1))
            ) {
                const r2 = bestR2;
                console.log("Smallest distance: ", smallestDistance, `${r1.polymerChainIdentifier}:${r1.sequenceNumber}${r1.name}`, `${r2.polymerChainIdentifier}:${r2.sequenceNumber}${r2.name}`);
            }else{
                console.log("No pair for: ", `${r1.polymerChainIdentifier}:${r1.sequenceNumber}${r1.name}`)
            }
        });
    }
